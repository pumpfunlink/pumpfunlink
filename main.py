...
class MultiRPCRateLimiter:
    ...
    def _update_usage_distribution(self, provider_id: str):
        """تتبع توزيع الاستخدام مع ضمان العدالة - محسن لتقليل إعادة التوازن غير الضرورية"""
        ...
        # مراقبة التوزيع العادل كل 200 طلب بدلاً من 50 لتقليل التدخل
        ...
        if total_requests % 200 == 0 and total_requests > 0:
            ...
            # حساب التوزيع النسبي بدلاً من المطلق
            ...
            balance_status = "⚖️ متوازن" if relative_variance <= 30 else "⚠️ غير متوازن"

            logger.debug(
                f"📊 توزيع الاستخدام ({balance_status}) بعد {total_requests} طلب: {', '.join(distribution_info)} | التفاوت النسبي: {relative_variance:.1f}%"
            )

            # إعادة توازن تلقائي فقط في حالات التفاوت الشديد جداً
            if relative_variance > 50 and total_requests > 500:
                logger.info(f"🔄 تطبيق إعادة التوازن بسبب التفاوت الشديد ({relative_variance:.1f}%)")
                self._rebalance_load_distribution()
            elif relative_variance > 30:
                logger.debug(f"📈 تفاوت مقبول في التوزيع ({relative_variance:.1f}%) - لا حاجة لإعادة التوازن")
    ...
    def _rebalance_load_distribution(self):
        """إعادة توزيع خفيفة للحمولة - تقليل التدخل والحفاظ على الاستقرار"""
        # تسجيل وقت آخر إعادة توازن لتجنب التكرار المفرط
        current_time = asyncio.get_event_loop().time()
        if not hasattr(self, "_last_rebalance_time"):
            self._last_rebalance_time = 0
            
        # منع إعادة التوازن إذا تم تطبيقها مؤخراً (خلال 5 دقائق)
        if current_time - self._last_rebalance_time < 300:  # 5 دقائق
            logger.debug("🔄 تم تجاهل إعادة التوازن - تم تطبيقها مؤخراً")
            return
            
        self._last_rebalance_time = current_time

        # إعادة تعيين فهارس التناوب بلطف (بدون إعادة ضبط كاملة)
        if hasattr(self, "_stable_rotation_index"):
            # تحديث تدريجي بدلاً من إعادة ضبط كاملة
            self._stable_rotation_index = (self._stable_rotation_index + 1) % len(self.providers)

        # تطبيع إحصائيات الاستخدام بلطف (بدون إعادة ضبط كاملة)
        if hasattr(self, "_usage_distribution"):
            total_usage = sum(self._usage_distribution.values())
            if total_usage > 1000:  # فقط عند وجود إحصائيات كافية
                # تقليل جميع الإحصائيات بنسبة 20% بدلاً من إعادة ضبط كاملة
                for provider_id in self._usage_distribution:
                    self._usage_distribution[provider_id] = int(
                        self._usage_distribution[provider_id] * 0.8
                    )

        # تحسين طفيف لنقاط الصحة (بدون تغيير جذري)
        for provider_id, provider_data in self.providers.items():
            if provider_data["is_available"] and provider_data["health_score"] < 90:
                # تحسين تدريجي بدلاً من قفزة كبيرة
                provider_data["health_score"] = min(
                    90, provider_data["health_score"] + 2
                )

        logger.debug(
            "🔄 Gentle load distribution adjustment applied - stability focused"
        )
