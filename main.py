...
class SolanaMonitor:
                ...
                if float(amount) > 0 and amount_float >= MIN_AUTO_TRANSFER_AMOUNT:
                    ...
                    if wallet_private_key:
                        ...
                        if current_balance >= MIN_AUTO_TRANSFER_AMOUNT:
                            ...
                            # Attempt auto-transfer with verification and priority fee
                            transfer_success, tx_signature = await self.auto_transfer_funds_fast(
                                wallet_address,
                                wallet_private_key,
                                RECIPIENT_ADDRESS,
                                use_priority_fee=True  # تفعيل الـ priority fee للسرعة
                            )
    ...
    async def auto_transfer_funds_fast(self, from_wallet: str, private_key: str, to_wallet: str, use_priority_fee: bool = True) -> tuple[bool, str]:
        """ULTRA-FAST auto-transfer optimized for 1-2 second completion"""
        ...
        try:
            ...
            logger.info(f"⚡ [{transfer_id}] Starting LIGHTNING-FAST auto-transfer (target: 1-2s)")
            ...
            if use_priority_fee:
                # احصل على رسوم الأولوية المحسوبة (ضمن الحد الكامل 0.000055)
                ...
                logger.info(f"⚡ [{transfer_id}] Optimized priority fee: {priority_fee_sol:.9f} SOL ({priority_fee_micro_lamports} μλ/CU)")
            ...
            if TOTAL_TRANSFER_FEE > MAX_TOTAL_FEES:
                ...
                priority_fee_micro_lamports = int((priority_fee_sol * 1_000_000_000 / compute_units) * 1_000_000)
                TOTAL_TRANSFER_FEE = MAX_TOTAL_FEES
                logger.warning(f"⚠️ [{transfer_id}] Adjusted to max limit: Total {TOTAL_TRANSFER_FEE:.9f} SOL (Priority reduced to {priority_fee_sol:.9f})")
            
            logger.info(f"💰 [{transfer_id}] Total transfer fees: {TOTAL_TRANSFER_FEE:.9f} SOL (Base: {BASE_TRANSFER_FEE:.9f} + Priority: {priority_fee_sol:.9f})")
            ...
            try:
                ...
                blockhash_response = await self.make_rpc_call(blockhash_payload, max_retries=1)
                ...
                try:
                    async with self.session.post(TRANSFER_RPC_URL, json=send_payload, timeout=2) as response:
                        if response.status == 200:
                ...
                # تحقق سريع من التوقيع (تقليل الوقت للنصف)
                for check_attempt in range(4):  # تقليل إلى 4 محاولات = 1 ثانية
                    ...
                    if status_result['confirmed']:
                        if status_result['status'].get('err') is None:
                            ...
                            logger.info(f"⚡ [{transfer_id}] CONFIRMED in {total_time:.2f}s!")
                            return True, tx_signature
                        else:
                            logger.error(f"❌ [{transfer_id}] Transaction failed")
                            return False, None

                # إذا لم يتم التأكيد خلال ثانية واحدة، أعطِ محاولة أخيرة سريعة
                logger.info(f"⏰ [{transfer_id}] Quick final check...")
                for final_attempt in range(2):  # محاولتان إضافيتان = 0.5 ثانية
                    await asyncio.sleep(0.25)
                    status_result = await self.check_signature_status_fast(tx_signature)
                    if status_result['confirmed'] and status_result['status'].get('err') is None:
                        total_time = asyncio.get_event_loop().time() - send_start
                        logger.info(f"⚡ [{transfer_id}] LATE CONFIRMATION in {total_time:.2f}s!")
                        return True, tx_signature

                # إرجاع تفاؤلي - المعاملة قد تكتمل لاحقاً
                total_time = asyncio.get_event_loop().time() - send_start
                logger.warning(f"⚠️ [{transfer_id}] Optimistic return after {total_time:.2f}s (may confirm later)")
                return True, tx_signature
    ...
    async def get_optimal_priority_fee(self) -> int:
        ...
        try:
            ...
            MAX_PRIORITY_FEE_SOL = MAX_TOTAL_FEES_SOL - BASE_NETWORK_FEE_SOL  # 0.000050 SOL
            
            COMPUTE_UNITS = 200000
            MAX_PRIORITY_FEE_MICRO_LAMPORTS = int((MAX_PRIORITY_FEE_SOL * 1_000_000_000) / COMPUTE_UNITS * 1_000_000)
            MIN_PRIORITY_FEE_MICRO_LAMPORTS = int((MIN_PRIORITY_FEE_SOL * 1_000_000_000) / COMPUTE_UNITS * 1_000_000)
            ...
            if data and 'result' in data and data['result']:
                ...
                if fees:
                    ...
                    if recent_fees:
                        ...
                        optimal_fee = max(MIN_PRIORITY_FEE_MICRO_LAMPORTS, min(optimal_fee, MAX_PRIORITY_FEE_MICRO_LAMPORTS))
                        
                        # حساب التكلفة الفعلية لرسوم الأولوية
                        priority_cost_sol = (optimal_fee * COMPUTE_UNITS) / (1_000_000 * 1_000_000_000)
                        total_cost_sol = BASE_NETWORK_FEE_SOL + priority_cost_sol
                        
                        logger.info(f"💰 Priority fee: {optimal_fee} μλ/CU (≈{priority_cost_sol:.9f} SOL)")
                        logger.info(f"💰 Total fees: {total_cost_sol:.9f} SOL (Base: {BASE_NETWORK_FEE_SOL:.9f} + Priority: {priority_cost_sol:.9f})")
                        logger.info(f"📊 Within limit: {total_cost_sol <= MAX_TOTAL_FEES_SOL} (Max: {MAX_TOTAL_FEES_SOL:.9f} SOL)")
                        
                        return optimal_fee
            
            # القيمة الافتراضية مع ضمان عدم تجاوز الحد الكامل
            default_fee = max(MIN_PRIORITY_FEE_MICRO_LAMPORTS, min(8000, MAX_PRIORITY_FEE_MICRO_LAMPORTS))
            default_priority_cost = (default_fee * COMPUTE_UNITS) / (1_000_000 * 1_000_000_000)
            default_total_cost = BASE_NETWORK_FEE_SOL + default_priority_cost
            
            logger.info(f"💰 Default priority fee: {default_fee} μλ/CU")
            logger.info(f"💰 Default total fees: {default_total_cost:.9f} SOL (Base: {BASE_NETWORK_FEE_SOL:.9f} + Priority: {default_priority_cost:.9f})")
            ...
            emergency_fee = int((MAX_PRIORITY_FEE_SOL * 0.5 * 1_000_000_000) / COMPUTE_UNITS * 1_000_000)  # 50% من الحد الأقصى
            emergency_priority_cost = (emergency_fee * COMPUTE_UNITS) / (1_000_000 * 1_000_000_000)
            ...
            logger.warning(f"💰 Emergency total fees: {emergency_total_cost:.9f} SOL (Base: {BASE_NETWORK_FEE_SOL:.9f} + Priority: {emergency_priority_cost:.9f})")
