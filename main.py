...
class SolanaWalletBot:
    ...
    async def _send_stats_message(
        ...
        try:
            ...
            stats_message += (
                f"• المزود الأمثل: {limiter_stats.get('optimal_provider', 'N/A')}\n\n"
            )
            ...
            if (
                hasattr(self.monitor, "cycle_start_time")
                and self.monitor.cycle_start_time
            ):
                current_cycle_time = (
                    asyncio.get_event_loop().time() - self.monitor.cycle_start_time
                )
            ...
            total_capacity = sum(
                p["max_rate"]
                for p in limiter_stats["providers"].values()
                if p["is_available"]
            )
