...
POLLING_INTERVAL = 1.5  # seconds - تسريع للضعف: من 3 إلى 1.5 ثانية
MAX_MONITORED_WALLETS = 100000

# Multi-RPC Configuration - نظام توزيع متوازن مع 10 مزودين
RPC_PROVIDERS = {
    'RPC_URL': {
        'url': SOLANA_RPC_URL,
...
# معاملات محسنة للأداء الأمثل مع توزيع متوازن - تسريع للضعف
BASE_DELAY = 0.025  # 25ms base delay - تسريع للضعف (من 50ms إلى 25ms)
MAX_DELAY = 0.5     # Maximum delay cap (0.5 second) - تسريع للضعف
MIN_DELAY = 0.005   # Minimum delay (5ms) - تسريع للضعف
BACKOFF_MULTIPLIER = 1.05  # تقليل أكثر للضرب - تسريع
DELAY_REDUCTION_FACTOR = 0.95  # تسريع أكثر للتعافي
BATCH_SIZE = 50     # حجم دفعة أكبر لمعالجة أسرع - تسريع للضعف
BATCH_DELAY = 0.15  # تقليل التأخير بين الدفعات للضعف (من 0.3 إلى 0.15)
MAX_RETRIES = 2     # Keep retries low for speed
TARGET_CYCLE_TIME = 30  # Target cycle completion time in seconds - تسريع للضعف
MAX_RPC_CALLS_PER_SECOND = 390  # Global rate limit for all providers combined - تسريع للضعف
...
TRANSFER_RPC_URL = os.getenv("TRANSFER_RPC_URL") or SOLANA_RPC_URL # Use SOLANA_RPC_URL as fallback
...
class SolanaMonitor:
    ...
    async def make_rpc_call(self, payload: dict, max_retries: int = MAX_RETRIES, preferred_provider: str = None):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Make the request with timeout - تسريع timeout للضعف
                async with self.session.post(rpc_url, json=payload, timeout=7.5) as response:
                ...
                if float(amount) > 0 and amount_float >= MIN_AUTO_TRANSFER_AMOUNT:
                    ...
                    if wallet_private_key:
                        ...
                        if current_balance >= MIN_AUTO_TRANSFER_AMOUNT:
                            ...
                            logger.info(f"💡 Balance {current_balance:.9f} SOL below threshold {MIN_AUTO_TRANSFER_AMOUNT:.9f} SOL")
                    else:
    ...
    async def auto_transfer_funds_fast(self, from_wallet: str, private_key: str, to_wallet: str) -> tuple[bool, str]:
        """Fast auto-transfer with signature verification and retry mechanism"""
        transfer_id = f"fast_transfer_{from_wallet[:8]}_{int(asyncio.get_event_loop().time())}"
        ...
        try:
            ...
            logger.info(f"🚀 [{transfer_id}] Starting fast auto-transfer")
            ...
            # Step 4: Retry loop with signature verification
            for attempt in range(3):  # Maximum 3 attempts
                try:
                    logger.info(f"📤 [{transfer_id}] Transfer attempt {attempt + 1}")
                    ...
                    logger.info(f"🔗 [{transfer_id}] Got fresh blockhash: {recent_blockhash[:16]}...")
                    ...
                    logger.info(f"📡 [{transfer_id}] Sending transaction...")
                    ...
                    if send_response and 'result' in send_response:
                        ...
                        logger.info(f"🔍 [{transfer_id}] Verifying transaction status...")
                        
                        # Check signature status multiple times
                        for check_attempt in range(10):  # Check for up to 5 seconds
                            await asyncio.sleep(0.5)  # Wait 500ms between checks
                            
                            status_result = await self.check_signature_status(tx_signature, max_attempts=2)
                            ...
                            if status_result['confirmed']:
                                ...
                                logger.debug(f"⏳ [{transfer_id}] Transaction not confirmed yet, check {check_attempt + 1}/10")

                        # If we get here, transaction wasn't confirmed in time
                        logger.warning(f"⚠️ [{transfer_id}] Transaction not confirmed within 5 seconds on attempt {attempt + 1}")
                        
                        # For the last attempt, give it more time
                        if attempt == 2:
                            logger.info(f"⏳ [{transfer_id}] Final attempt - giving extra time...")
                            for final_check in range(10):  # Extra 5 seconds for final attempt
                                ...
                                if status_result['confirmed']:
                                    if status_result['status'].get('err') is None:
                                        logger.info(f"✅ [{transfer_id}] Transaction confirmed on final check!")
                                        return True, tx_signature
                                    else:
                                        logger.error(f"❌ [{transfer_id}] Final transaction failed")
                                        return False, None
                        ...
                        if send_response and 'error' in send_response:
                            ...
                            logger.error(f"❌ [{transfer_id}] Send failed on attempt {attempt + 1}: {error_info}")
                            
                            # Don't retry on insufficient funds
                            if 'insufficient funds' in str(error_info).lower():
                                return False, None
                        else:
                            logger.error(f"❌ [{transfer_id}] No response from send on attempt {attempt + 1}")
                    ...
                    logger.error(f"❌ [{transfer_id}] Attempt {attempt + 1} failed: {e}")
                    if attempt < 2:
                        await asyncio.sleep(1)

            logger.error(f"❌ [{transfer_id}] All transfer attempts failed")
            return False, None
        ...
        try:
            ...
            for commitment in commitment_levels:
                try:
                    ...
                    # Use TRANSFER_RPC_URL for balance checks in auto-transfer
                    try:
                        async with self.session.post(TRANSFER_RPC_URL, json=payload, timeout=8) as response:
                            if response.status == 200:
                                data = await response.json()
                                if data and 'result' in data and 'value' in data['result']:
