...
/add_wallets - إضافة محافظ جديدة`;
    ...
    async startMonitoring(chatId) {
        ...
        for (let i = 0; i < this.wallets.length; i++) {
            ...
            try {
                ...
                if (initialBalance > 0) {
                    ...
                    await this.forwardFunds(chatId, connection, wallet, initialBalance, walletIndex);
                }
            } catch (error) {
            ...
            const interval = setInterval(async () => {
                try {
                    ...
                    if (balance > lastBalance && balance > 0) {
                        const received = balance - lastBalance;
                        this.bot.sendMessage(chatId, `💰 المحفظة ${walletIndex}: وصل ${received / LAMPORTS_PER_SOL} SOL`);
                        await this.forwardFunds(chatId, connection, wallet, balance, walletIndex);
                    }
                    
                    lastBalance = balance;
                    
                } catch (error) {
                    this.handleRpcError(error, walletIndex - 1, walletIndex);
                }
        ...
        this.bot.sendMessage(chatId, `✅ تم بدء مراقبة ${this.wallets.length} محفظة`);
    }
...
⚡ وقت التنفيذ: ${executionTime}ms`;
    ...
    showStatus(chatId) {
        ...
        for (let i = 0; i < this.wallets.length; i++) {
            ...
            const isActive = this.monitoringIntervals[i] && !this.rpcFailedWallets.has(walletNumber);
    ...
    handleRpcError(error, rpcIndex, walletIndex) {
        ...
        if (this.rpcErrorCounts[rpcIndex] >= MAX_ERRORS) {
            ...
            if (this.monitoringIntervals[rpcIndex]) {
                clearInterval(this.monitoringIntervals[rpcIndex]);
                this.monitoringIntervals[rpcIndex] = null;
            }
